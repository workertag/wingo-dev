...
import { Coins, Flame, Landmark, Network } from "lucide-react";
...
export function AdminAutoPoolPage() {
  const [page, setPage] = useState(1);

  const { data: treasury } = useQuery({
    queryKey: ["admin-auto-pool-treasury"],
    queryFn: async () => {
      const res = await api.admin.pool["2x2"].treasury.get();
      return res.data?.data ?? null;
    },
  });

  const { data: positions, isLoading } = useQuery({
    queryKey: ["admin-auto-pool-positions", page],
    queryFn: async () => {
      const res = await api.admin.pool["2x2"].positions.get({
        query: { page, size: 20 },
      });
      return res.data?.data ?? null;
    },
    ...
    <div className="relative overflow-hidden min-h-screen bg-[#faf8f6] text-slate-800">
      ...
      <div className="relative z-10 max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="relative overflow-hidden rounded-3xl bg-white/95 backdrop-blur-md border border-amber-200/85 p-6 shadow-md shadow-amber-950/5">
          <div className="relative z-10 flex items-center gap-4.5">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-tr from-amber-50 via-orange-100/50 to-amber-200 border border-amber-300 shadow-md">
              <Network className="w-8 h-8 text-[var(--gmc-gold)]" />
            </div>
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-slate-600 font-semibold">
                Treasury totals and all placed positions. Entry price, rewards
                and enabled entry methods are edited in System Config.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <StatCard
            icon={Flame}
            label="Burned Value (Reporting)"
            value={dollars(treasury?.burnedValueCents)}
          />
          <StatCard
            icon={Coins}
            label="Paid Out"
            ...
            <table className="w-full text-xs sm:text-sm">
              <thead>
                <tr className="text-left text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-amber-100">
                  ...
                  <th className="py-2 pr-4">Entry Method</th>
                ...
                {isLoading ? (
                  ...
                  positions?.list.map((p) => (
                    <tr key={p.id} className="border-b border-amber-50">
                      ...
                      <td className="py-2 pr-4">{p.cycleNumber}</td>
                      <td className="py-2 pr-4 capitalize">
                        {p.entryMethod.replace("_", " ")}
                      </td>
