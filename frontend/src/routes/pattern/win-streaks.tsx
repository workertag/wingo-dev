...
import { TrendingUp, Zap, Calendar, X, Clock } from "lucide-react";
import { LoginScreen } from "@/components/LoginScreen";

export const Route = createFileRoute("/pattern/win-streaks")({
  component: WinStreaksPage,
});
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 pb-6 border-b border-slate-200/60 mt-4">
        <div>
          ...
          <h2 className="text-4xl font-black text-indigo-600 tracking-tight leading-none mb-2 flex items-center gap-3">
            Win-Streak <span className="text-slate-900">Monitor</span>
          </h2>
          <p className="text-sm font-medium text-slate-500">
            Track consecutive WIN occurrences for both AI Engines across different history windows.
          </p>
        </div>
        ...
        <div className="flex p-1.5 bg-white rounded-full shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-slate-100">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-full text-sm font-bold transition-all ${
              activeTab === "30S" ? "bg-gradient-to-r from-indigo-500 to-violet-500 text-white shadow-md" : "text-slate-500 hover:text-slate-800"
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-full text-sm font-bold transition-all ${
              activeTab === "1M" ? "bg-gradient-to-r from-indigo-500 to-violet-500 text-white shadow-md" : "text-slate-500 hover:text-slate-800"
            }`}
          >
            ...
            <table className="w-full min-w-[800px] border-collapse">
              <thead>
                <tr className="bg-slate-50/50">
                  <th className="py-3 px-4 text-left font-black text-xs text-slate-400 uppercase tracking-widest border-b border-slate-100 sticky left-0 bg-slate-50/95 backdrop-blur z-10 w-32">
                    Engine
                  </th>
                  <th colSpan={windows.length} className="py-2 px-2 text-center font-black text-xs text-indigo-600 bg-indigo-50/50 uppercase tracking-widest border-b border-l border-slate-100">
                    Big / Small
                  </th>
                  <th colSpan={windows.length} className="py-2 px-2 text-center font-black text-xs text-emerald-600 bg-emerald-50/50 uppercase tracking-widest border-b border-l border-slate-100">
                    Red / Green
                  </th>
                </tr>
                <tr className="bg-white border-b border-slate-100 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  <td className="py-3 px-4 sticky left-0 bg-white z-10">Streak Length</td>
                  {windows.map((w, i) => <td key={`bs-win-${w}`} className="py-3 px-2 border-l border-slate-50 w-[50px]">{windowLabels[i]}</td>)}
                  {windows.map((w, i) => <td key={`rg-win-${w}`} className={`py-3 px-2 w-[50px] ${i === 0 ? 'border-l border-slate-100' : 'border-l border-slate-50'}`}>{windowLabels[i]}</td>)}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-slate-600">
                {streakRows.map(streakLen => (
                  <tr key={streakLen} className="hover:bg-slate-50/50 transition-colors">
                    <td className="py-3.5 px-4 text-left font-bold text-slate-700 sticky left-0 bg-white/95 backdrop-blur z-10">
                      {streakLen === 15 ? '15+ wins' : `${streakLen} win${streakLen > 1 ? 's' : ''}`}
                    </td>
                    {windows.map((w, i) => {
                      let count = 0;
                      let times: number[] = [];
            ...
            <div className="p-5 sm:p-6 border-b border-slate-100 flex items-start justify-between bg-slate-50/50">
              <div>
                <h3 className="text-lg font-black text-slate-800 tracking-tight flex items-center gap-2">
                  ...
                  {modalData.streak === 15 ? '15+' : modalData.streak}-Win Streak Timing
                </h3>
